...
partial_dat %>% 
  ...
  geom_boxplot() +
  geom_point(aes(x = Treatment, y = abundance, colour = Mes_ID),
             position = "jitter", 
             alpha = 0.4,
             # shape = 1, stroke = 0.7
             ) +
...
mb = brm(bf(HF_total/Vol \~ 0 + Treatment + (1\|Mesocosm)), chains = 4, iter = 20000, cores = 4, adapt_delta = 0.99, backend = "cmdstanr", data = partial_dat)

pp_check(mb, ndraws = 1e2) plot(mb) summary(mb)
