...
some ingestion rate dataframes
```{r, echo=F}
...
working <- ingest %>% 
  group_by(Treatment, Mes_ID, Sample, Time_point, Grazer) %>% 
  ...
  select(-contains("cell_num_T"), -contains("Tstart")) 

#maybe need to replace with 0, will see
working[sapply(working, is.infinite)] <- NA
